...
public class IntakeSimple extends SubsystemBase {
  ...
  public IntakeSimple() {
    ...
    m_motor = new CANSparkMax(IntakeConstants.kCanId, MotorType.kBrushless);
    checkRevError(m_motor.restoreFactoryDefaults());

    m_motor.setInverted(false);
    checkRevError(m_motor.setSmartCurrentLimit(IntakeConstants.kCurrentLimit));
    checkRevError(m_motor.setIdleMode(IdleMode.kBrake));

    m_encoder = m_motor.getEncoder(SparkRelativeEncoder.Type.kHallSensor, 42);

    m_controller = m_motor.getPIDController();
    PIDGains.setSparkMaxGains(m_controller, IntakeConstants.kPositionGains);

    checkRevError(m_motor.burnFlash());
   ...
   */
  public Command feedLauncher(ShooterSimple _launcher) {
    Command newCommand =
        new Command() {
          ...
          public void execute() {
            setPower(1.0);
            _launcher.runShooter();
          }

          @Override
          public boolean isFinished() {
            return m_timer.get() > IntakeConstants.kShotFeedTime;
          }
  ...
  public Command retract() {
    Command newCommand =
        new Command() {
          ...
          public void initialize() {
            m_positionMode = true;
            m_targetPosition = m_encoder.getPosition() + IntakeConstants.kRetractDistance;
          }
  ...
  public boolean isNearTarget() {
    return Math.abs(m_encoder.getPosition() - m_targetPosition)
        < IntakeConstants.kPositionTolerance;
  }
}
